...
const main = Effect.gen(function* () {
  ...
  const solutionsJson = yield* Schema.encodeEffect(SolutionsFromJson)(uniqueWords).pipe(
    Effect.mapError((cause) => new SolutionsJsonEncodeError({ file: "./src/seed/solutions-pl.json", cause })),
    Effect.orDie
  );

  // Write to the new JSON file
  yield* fs.writeFileString("./src/seed/solutions-pl.json", solutionsJson);
