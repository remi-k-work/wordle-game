...
class SolutionsJsonEncodeError extends Schema.TaggedError<SolutionsJsonEncodeError>()("SolutionsJsonEncodeError", {
  file: Schema.String,
  cause: Schema.Defect(),
}) {}
...
const SolutionsFromJson = Schema.fromJsonString(SolutionsSchema, { space: 2 });
...
const main = Effect.gen(function* () {
  ...
  const solutionsJson = yield* Schema.encodeEffect(SolutionsFromJson)(uniqueWords).pipe(
    Effect.mapError((cause) => new SolutionsJsonEncodeError({ file: "./src/seed/solutions-en.json", cause })),
    Effect.orDie
  );

  // Write to the new JSON file
  yield* fs.writeFileString("./src/seed/solutions-en.json", solutionsJson);
