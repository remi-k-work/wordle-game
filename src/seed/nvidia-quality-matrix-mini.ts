...
// =============================================================================
// Mini-matrix: 4 actually-available models on https://integrate.api.nvidia.com/v1
// Control + 3 candidates that ARE in the live /v1/models list (verified via curl)
// =============================================================================
...
const QUALITY_MODELS: readonly QualityModel[] = [
  // Control — proven 4/4 inside 60s with thinking OFF in full matrix
  { model: "nvidia/nemotron-3-super-120b-a12b", label: "Nemotron Super 120B (thinking OFF)", langHint: "Control — reasoning disabled", disableThinking: true },
  // Euro-ish candidates that ARE in /v1/models (not the 404 bielik/eurollm IDs)
  { model: "mistralai/mistral-7b-instruct-v0.3", label: "Mistral 7B v0.3", langHint: "Euro 7B — available", disableThinking: false },
  { model: "google/gemma-3-12b-it", label: "Gemma 3 12B IT", langHint: "Google 12B — available", disableThinking: false },
  { model: "nvidia/llama-3.1-nemotron-70b-instruct", label: "Llama Nemotron 70B", langHint: "NVIDIA Llama 70B — available", disableThinking: false },
] as const;

// =============================================================================
// Prompt builders — mirrored from riddle2.ts / override2.ts
// =============================================================================
...
const main = Effect.gen(function* () {
  ...
  yield* Effect.log("🔬 NVIDIA Quality Matrix MINI — 4 actually-available models");
  ...
  yield* Effect.tryPromise(() => Fs.writeFile("src/seed/nvidia-quality-matrix-mini.report.md", md, "utf8")).pipe(
    Effect.catch((e: unknown) => Effect.logError(`Failed to write md report: ${String(e)}`))
  );
  yield* Effect.tryPromise(() => Fs.writeFile("src/seed/nvidia-quality-matrix-mini.report.json", json, "utf8")).pipe(
    Effect.catch((e: unknown) => Effect.logError(`Failed to write json report: ${String(e)}`))
  );
  yield* Effect.log("\nReports written: src/seed/nvidia-quality-matrix-mini.report.md + .json");
