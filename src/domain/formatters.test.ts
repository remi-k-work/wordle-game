...
describe("formatters", () => {
  ...
  describe("speedMultiplierToCategory", () => {
    it("returns correct English categories", () => {
      expect(speedMultiplierToCategory("En", 1.5)).toBe("Speed Demon");
      expect(speedMultiplierToCategory("En", 0.8)).toBe("Slow Learner");
    });

    it("returns correct Polish categories", () => {
      expect(speedMultiplierToCategory("Pl", 1.5)).toBe("Demon Szybkości");
      expect(speedMultiplierToCategory("Pl", 0.8)).toBe("Powolny Uczeń");
    });
  });
});
